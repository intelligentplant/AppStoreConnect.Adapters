...
namespace DataCore.Adapter.Subscriptions {
    public static class SubscriptionTopicFilterComparer {
        ...
        public static unsafe SubscriptionTopicFilterCompareResult Compare(string topic, string filter) {
            if (string.IsNullOrEmpty(topic)) {
                return SubscriptionTopicFilterCompareResult.TopicInvalid;
            }

            if (string.IsNullOrEmpty(filter)) {
                return SubscriptionTopicFilterCompareResult.FilterInvalid;
            }
            ...
            fixed (char* filterPointer = filter) {
                if (filterLength > topicLength) {
                    ...
                    if (lastFilterChar != MultiLevelWildcard && lastFilterChar != SingleLevelWildcard) {
                        return SubscriptionTopicFilterCompareResult.NoMatch;
                    }
                }
                ...
                if (filterLength == 1 && isMultiLevelFilter) {
                    ...
                    return SubscriptionTopicFilterCompareResult.IsMatch;
                }
